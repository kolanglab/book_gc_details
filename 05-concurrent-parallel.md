...
[Baker](#cite:baker1978)は、ミューテータとGCが交互に動作するインクリメンタルなコピーGCを提案した。ミューテータがオブジェクトにアクセスする際、そのオブジェクトがまだfrom-spaceにある場合はto-spaceにコピーしてからアクセスする（リードバリア）。Bakerのアルゴリズムは直接的に採用した処理系は少ないが、リードバリアの概念は後のShenandoah（Brooksポインタ）やZGC（ロードバリア）に大きな影響を与えた。Erlang VMの初期のGC設計にも影響を与えている。
