...
本書で見てきたように、GCは60年以上にわたる研究の蓄積を持つ成熟した分野でありながら、2026年現在もなお活発な研究が続いている。本章では、今後10年のGC研究と実装の方向性を展望する。
...
**Perceusの拡張**: [Reinking et al.](#cite:reinking2021)のPerceusは関数型言語向けだが、そのアイデアをオブジェクト指向言語に適用する研究が進んでいる。借用チェック（Rustスタイル）と参照カウントの組み合わせにより、より広い範囲のプログラムでGCなし実行が可能になるかもしれない。
...
ErlangやPonyのようなアクターモデル言語では、アクターごとに独立したヒープを持つことで、GCの影響を局所化できる。[Ueno and Ohori](#cite:ueno2016)はマルチコア環境での関数型プログラムの完全並行GCを提案しており、このアプローチはアクターモデルと自然に調和する。
...
GC研究は、理論と実践が密接に結びついた分野である。[Jones, Hosking, Moss](#cite:jones2023)の教科書が示すように、基礎理論の深い理解が優れた実装を生み、実装の経験が新しい理論を触発する。
